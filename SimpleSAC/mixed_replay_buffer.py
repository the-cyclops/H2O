...
# Differenze dataset: 
# medium rappresenta un dataset collezionato da una policy statica già trainata, abbiamo quindi alta qualità ma poca esplorazione
# medium_replay rappresenta un dataset collezionato da una policy nel processo di training, abbiamo quindi bassa qualità ma alta esplorazione
...
class MixedReplayBuffer(ReplayBuffer):
	def __init__(self, reward_scale, reward_bias, clip_action, state_dim, action_dim, task="halfcheetah", data_source="medium_replay",  device="cuda", scale_rewards=True, scale_state=False, buffer_ratio=1, residual_ratio=0.1):
		...
		with h5py.File(path, "r") as dataset:
			...
			# OBSERVATIONS: velocità, posizione, angoli articolazioni di HalfCheetah --> input della rete neurale
