...
def main(argv):
	...
	# discriminator, to approximate the real and simulated data
	# d_sa is the discriminator for state-action pairs, d_sas is the discriminator for state-action-state pairs
	...
	# target_qf1 is the target network for qf1, used for soft updates
	...
	# target_qf2 is the target network for qf2, used for soft updates
	target_qf2 = deepcopy(qf2)

	if FLAGS.cql.target_entropy >= 0.0:
		FLAGS.cql.target_entropy = -np.prod(eval_sampler.env.action_space.shape).item()

	# create the Sim2realSAC object
		...
		with Timer() as train_timer:
			for batch_idx in trange(FLAGS.n_train_step_per_epoch):
				...
				if batch_idx + 1 == FLAGS.n_train_step_per_epoch:
					metrics.update(
						prefix_metrics(sac.train(real_batch_size, sim_batch_size), 'sac') # metodo 'train' di sim2real_sac class
					)
				else:
					sac.train(real_batch_size, sim_batch_size)

		# TODO Evaluate in the real world
		# Runs the policy in the real environment via eval_sampler
